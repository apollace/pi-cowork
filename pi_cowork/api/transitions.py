...
def api_create_transition():
    ...
    try:
        ...
        add_log('INFO', 'db_change', f'INSERT transitions/{cur.lastrowid}', details={'operation': 'INSERT', 'table': 'transitions', 'record_id': cur.lastrowid})
...
def api_update_transition(transition_id):
    ...
    run_db(
        "UPDATE transitions SET instructions = ? WHERE id = ?",
        ((instructions or '').strip() or None, transition_id)
    )
    add_log('INFO', 'db_change', f'UPDATE transitions/{transition_id}', details={'operation': 'UPDATE', 'table': 'transitions', 'record_id': transition_id})
...
def api_delete_transition(transition_id):
    ...
    add_log('INFO', 'db_change', f'DELETE transitions/{transition_id}', details={'operation': 'DELETE', 'table': 'transitions', 'record_id': transition_id})
    return jsonify({"success": True})