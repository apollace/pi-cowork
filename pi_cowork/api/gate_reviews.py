...
from pi_cowork.models import add_comment, get_status, get_agent, get_board, get_agents, get_pending_gate_reviews
from pi_cowork.agents import try_spawn_or_queue
from pi_cowork.events import bus, GATE_PASSED, GATE_FAILED, TICKET_STATUS_CHANGED
...
def api_update_gate_review(review_id):
    ...
    if not secret or provided != secret:
        add_log('WARNING', 'http_request', f'Gate review approval blocked — missing or invalid X-Human-Action header (review_id={review_id})')
    ...
    add_log('INFO', 'db_change', f'UPDATE gate_reviews/{review_id}', details={'operation': 'UPDATE', 'table': 'gate_reviews', 'record_id': review_id}, ticket_id=ticket_id)
    ...
    if new_status == 'approved':
        ...
        if not pending:
            ...
            if not any_rejected:
                ...
                run_db("UPDATE tickets SET status_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", (new_status_id, ticket_id))
                add_log('INFO', 'db_change', f'UPDATE tickets/{ticket_id}', details={'operation': 'UPDATE', 'table': 'tickets', 'record_id': ticket_id, 'via': 'gate_approval'}, ticket_id=ticket_id)
                ...
                if status and status.get('is_terminal'):
                    ...
                    ticket_row = query_db("SELECT t.board_id, b.workflow_id FROM tickets t JOIN boards b ON t.board_id = b.id WHERE t.id = ?", (ticket_id,), one=True)
                    if ticket_row:
