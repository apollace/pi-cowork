...
from pi_cowork.models import (
    ...
    get_agent,
    get_board,
    get_board_with_workflow,
    get_comment_counts,
    get_comments,
    get_quality_gates,
    get_recurring_parents,
...
def api_ticket(ticket_id):
    ...
    q = query_db("SELECT reason FROM agent_queue WHERE ticket_id = ? AND started_at IS NULL", (ticket_id,), one=True)
...
def api_create_ticket():
    ...
    cur = run_db(
        ...
        (title, body, status_id, board_id, priority),
    )
    ...
    add_log(
        "INFO",
        "db_change",
        f"INSERT tickets/{ticket_id}",
        details={"operation": "INSERT", "table": "tickets", "record_id": ticket_id},
        ticket_id=ticket_id,
    )
    ...
    return jsonify(
        {"id": ticket_id, "status_id": status_id, "board_id": board_id, "labels": get_ticket_labels(ticket_id)}
    ), 201
            ...
            if not all_resolved:
                ...
                if updates:
                    ...
                    add_log(
                        ...
                        details={"operation": "UPDATE", "table": "tickets", "record_id": ticket_id},
                        ticket_id=ticket_id,
                    )
                add_comment(
                    ticket_id,
                    f"⏳ Quality gate pending — transition to '{dest_status['name']}' awaits manual approval.",
                )
...
def api_spawn_agent(ticket_id):
    ...
    running_row = query_db(
        "SELECT COUNT(*) AS c FROM agent_runs WHERE ticket_id = ? AND status = 'running'", (ticket_id,), one=True
    )
    ...
    queued_after = query_db(
        "SELECT COUNT(*) AS c FROM agent_queue WHERE ticket_id = ? AND started_at IS NULL", (ticket_id,), one=True
    )
