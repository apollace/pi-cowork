...
@tickets_bp.route('/api/tickets/<int:ticket_id>/ask', methods=['POST'])
def api_ask_agent(ticket_id):
    """Ask the assigned agent a question (Q&A mode).

    The agent is spawned in ask mode: it can read the ticket and post a
    single comment with the answer, but MUST NOT change the ticket status,
    title, or body. The spawn uses the same session dir as work runs so
    prior turn memory is preserved.
    """
    ticket = query_db("""
        SELECT t.*, b.workflow_id, b.name AS board_name, w.name AS workflow_name, b.working_directory
        FROM tickets t
    ...
    if requested_agent_id is not None:
        try:
            requested_agent_id = int(requested_agent_id)
        except (TypeError, ValueError):
            return jsonify({"error": "agent_id must be an integer"}), 400
        agent = get_agent(requested_agent_id)
        if not agent:
            return jsonify({"error": "Agent not found"}), 404
        if int(agent['workflow_id']) != int(ticket['workflow_id']):
            return jsonify({"error": "Agent does not belong to this ticket's workflow"}), 409
    else:
        agent_id = status.get('agent_id')
        if not agent_id:
            return jsonify({"error": "Cannot ask agent: current status has no agent assigned"}), 409
        agent = get_agent(agent_id)
        if not agent:
            return jsonify({"error": "Cannot ask agent: agent not found"}), 409

    # Guard: an ask-mode run is already running on this ticket.
    # (Multiple work runs are already blocked by the parallel limit, but
    # this adds an extra one-at-a-time guard for ask specifically.)
    ask_running_row = query_db(
        "SELECT COUNT(*) AS c FROM agent_runs WHERE ticket_id = ? AND status = 'running' AND mode = 'ask'",
        (ticket_id,), one=True
    )
    if ask_running_row and ask_running_row['c'] > 0:
        return jsonify({"error": "Cannot ask agent: an ask run is already in progress on this ticket"}), 409

    ask_system_prompt = agent.get('ask_system_prompt')
    ...
    # Check whether the ask was actually spawned or blocked
    running_after = query_db(
        "SELECT COUNT(*) AS c FROM agent_runs WHERE ticket_id = ? AND status = 'running' AND mode = 'ask'",
        (ticket_id,), one=True
    )
