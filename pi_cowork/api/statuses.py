...
def api_create_status():
    ...
    try:
        ...
        add_log('INFO', 'db_change', f'INSERT statuses/{cur.lastrowid}', details={'operation': 'INSERT', 'table': 'statuses', 'record_id': cur.lastrowid})
...
def api_update_status(status_id):
    ...
    try:
        ...
        return jsonify({"error": "Status name already exists"}), 409
    add_log('INFO', 'db_change', f'UPDATE statuses/{status_id}', details={'operation': 'UPDATE', 'table': 'statuses', 'record_id': status_id})
...
def api_delete_status(status_id):
    ...
    in_use_transition = query_db("SELECT 1 FROM transitions WHERE from_status_id = ? OR to_status_id = ? LIMIT 1", (status_id, status_id), one=True)
    ...
    add_log('INFO', 'db_change', f'DELETE statuses/{status_id}', details={'operation': 'DELETE', 'table': 'statuses', 'record_id': status_id})
    return jsonify({"success": True})