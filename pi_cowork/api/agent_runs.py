...
def api_kill_agent_run(run_id):
    ...
    if not _agents_mod._is_our_process(pid):
        ...
        add_log('WARNING', 'agent_event', f'Agent run {run_id} killed by user (process already terminated)', details={'agent_name': run.get('agent_name', 'Unknown'), 'run_id': run_id, 'exit_code': -15}, ticket_id=ticket_id)
    ...
    add_log('WARNING', 'agent_event', f'Agent run {run_id} killed by user', details={'agent_name': run.get('agent_name', 'Unknown'), 'run_id': run_id, 'exit_code': exit_code, 'escalated': escalated}, ticket_id=ticket_id)
