...
from pi_cowork.models import get_workflow, get_label, get_labels, get_ticket_labels, set_ticket_labels
...
def api_create_label():
    ...
    try:
        ...
        add_log('INFO', 'db_change', f'INSERT labels/{cur.lastrowid}', details={'operation': 'INSERT', 'table': 'labels', 'record_id': cur.lastrowid})
...
def api_update_label(label_id):
    ...
    try:
        ...
        return jsonify({"error": "Label name already exists in this workflow"}), 409
    add_log('INFO', 'db_change', f'UPDATE labels/{label_id}', details={'operation': 'UPDATE', 'table': 'labels', 'record_id': label_id})
...
def api_delete_label(label_id):
    ...
    add_log('INFO', 'db_change', f'DELETE labels/{label_id}', details={'operation': 'DELETE', 'table': 'labels', 'record_id': label_id})
    return jsonify({"success": True})
