...
from pi_cowork.models import get_workflow, get_board, get_board_with_workflow
...
def api_create_board():
    ...
    try:
        cur = run_db("INSERT INTO boards (name, workflow_id, working_directory, long_term_vision) VALUES (?, ?, ?, ?)", (name, workflow_id, str(wd_path.resolve()), long_term_vision))
        add_log('INFO', 'db_change', f'INSERT boards/{cur.lastrowid}', details={'operation': 'INSERT', 'table': 'boards', 'record_id': cur.lastrowid})
...
def api_update_board(board_id):
    ...
    try:
        ...
        return jsonify({"error": "Board name already exists"}), 409
    add_log('INFO', 'db_change', f'UPDATE boards/{board_id}', details={'operation': 'UPDATE', 'table': 'boards', 'record_id': board_id})
...
def api_delete_board(board_id):
    ...
    add_log('INFO', 'db_change', f'DELETE boards/{board_id}', details={'operation': 'DELETE', 'table': 'boards', 'record_id': board_id})
    return jsonify({"success": True})