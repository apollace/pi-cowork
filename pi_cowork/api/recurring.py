...
def api_trigger_recurring(task_id):
    ...
    add_comment(
        ticket_id,
        f"🔄 Ticket manually triggered from recurring task **{task['title']}** (schedule: `{task['cron_expression']}`)",
    )

    bus.publish("recurring.triggered", recurring_task_id=task_id, ticket_id=ticket_id, board_id=task["board_id"])
    bus.publish(
        TICKET_CREATED, ticket_id=ticket_id, title=ticket_title, board_id=task["board_id"], status_id=task["status_id"]
    )
