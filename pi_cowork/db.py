...
def _migrate(db):
    ...
    migrations = [
        ...
        ('add_boards_long_term_vision',
         'ALTER TABLE boards ADD COLUMN long_term_vision TEXT'),
        ...
        ('add_agents_model_column',
         'ALTER TABLE agents ADD COLUMN model TEXT'),
        ('add_agents_thinking_column',
        ...
        ('add_agents_api_endpoints',
         'ALTER TABLE agents ADD COLUMN api_endpoints TEXT'),
        ...
        ('seed_max_parallel',
         "INSERT OR IGNORE INTO settings (key, value) VALUES ('max_parallel', '1')"),
        ('seed_max_per_hour',
         "INSERT OR IGNORE INTO settings (key, value) VALUES ('max_per_hour', '100')"),
        ('seed_warm_spawn_threshold',
         "INSERT OR IGNORE INTO settings (key, value) VALUES ('warm_spawn_threshold', '3600')"),
        ('seed_run_max_age',
         "INSERT OR IGNORE INTO settings (key, value) VALUES ('run_max_age', '7200')"),
        # Ticket #67 — Configurable port
        ('seed_port',
         "INSERT OR IGNORE INTO settings (key, value) VALUES ('port', '5000')"),
        # Ticket #59 — Store old_status_id in agent_queue for transition context
        ('add_agent_queue_old_status_id',
         'ALTER TABLE agent_queue ADD COLUMN old_status_id INTEGER'),
        # Ticket #63 — Add priority to tickets
        ('add_tickets_priority',
         "ALTER TABLE tickets ADD COLUMN priority TEXT DEFAULT 'Medium'"),
        ('backfill_tickets_priority',
         "UPDATE tickets SET priority = 'Medium' WHERE priority IS NULL"),
        # Ticket #64 — Assistant per-endpoint API docs
        ('add_assistant_api_endpoints',
         'ALTER TABLE assistant_config ADD COLUMN api_endpoints TEXT'),
        ...
        ('add_statuses_model_column',
         'ALTER TABLE statuses ADD COLUMN model TEXT'),
...
def init_db(app):
    ...
    with app.app_context():
        db = get_db()
        with open(_ORIGINAL_SCHEMA_PATH, 'r') as f:
