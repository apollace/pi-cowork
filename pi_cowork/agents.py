...
from pi_cowork.db import get_db, row_to_dict, run_db, query_db
from pi_cowork.models import (
    add_comment, count_unanswered_questions, get_agent,
    get_board, get_comments, get_quality_gates, get_status,
    get_transitions_from, has_pending_gate_reviews, _add_question_wait_comment,
)
...
def count_hourly():
    row = query_db(
        "SELECT COUNT(*) AS c FROM agent_runs WHERE started_at > datetime('now', '-1 hour')",
        one=True
    )
    return row['c'] if row else 0
...
def drain_queue():
    ...
    pending = query_db(
        "SELECT * FROM agent_queue WHERE started_at IS NULL ORDER BY queued_at"
    )
    for q in pending:
...
def spawn_agent(ticket, status, agent, old_status_id=None):
    ...
    transitions_line = "Next status you MUST set (pick exactly one):\n" + "\n".join(transition_parts) if transition_parts else ""
    ...
    api_docs = build_api_docs(
        selected_keys, ticket_id, base_url=get_config('pi_cowork_url'), has_gates=has_gates,
    ...
    if is_warm:
            ...
            for c in comments:
                ...
                if c_time and isinstance(c_time, str):
                    ...
                    if c_time_dt.tzinfo is None:
                        c_time_dt = c_time_dt.replace(tzinfo=timezone.utc)
                else:
                    c_time_dt = None
        ...
        if status_changed:
            ...
            goal_instruction = f"This is a new prompt, forget the goals you had from previous prompts.\nYour goal: {goal_line}"
        else:
...
After completing your task, write a comment on the ticket summarizing what you did."""
    ...
    try:
        ...
        try:
            ...
            pass
        db.execute(
            "UPDATE agent_runs SET status = 'failed', pid = NULL WHERE id = ?",
            (run_id,)
        )
        db.commit()
