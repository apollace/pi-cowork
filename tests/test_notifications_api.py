...
def _create_agent(client, workflow_id, name="Agent"):
    res = client.post("/api/agents", json={"name": name, "description": "test", "workflow_id": workflow_id})
    return json.loads(res.data)["id"]
...
def _create_question(client, ticket_id, body="Q?"):
    ...
    with client.application.app_context():
        cur = run_db(
            "INSERT INTO questions (ticket_id, body, created_at) VALUES (?, ?, datetime('now'))", (ticket_id, body)
        )
        return cur.lastrowid
...
def _create_terminal_status(client, workflow_id, name="Done", sort_order=99):
    ...
    res = client.post(
        "/api/statuses", json={"name": name, "sort_order": sort_order, "workflow_id": workflow_id, "is_terminal": True}
    )
    return json.loads(res.data)["id"]
...
class TestTerminalStateNotificationClearing:
    ...
    def test_gate_approval_to_terminal_clears_questions(self, client):
        ...
        res = client.put(
            f"/api/gate_reviews/{rid}", json={"status": "approved", "comment": "Looks good"}, headers=HUMAN_HEADERS
        )
        assert res.status_code == 200
...
class TestResolveEndpointRemoved:
    ...
    def test_resolve_endpoint_returns_404(self, client):
        res = client.put("/api/notifications/resolve", json={"ticket_id": 1, "type": "gate_review"})
...
class TestDismissSingleNotification:
    ...
    def test_dismiss_gate_review_notification(self, client):
        ...
        # Dismiss it
        res = client.put("/api/notifications/dismiss", json={"ticket_id": ticket, "type": "gate_review"})
        assert res.status_code == 200
    ...
    def test_dismiss_question_notification(self, client):
        ...
        res = client.put("/api/notifications/dismiss", json={"ticket_id": ticket, "type": "question"})
        assert res.status_code == 200
    ...
    def test_dismiss_invalid_type(self, client):
        res = client.put("/api/notifications/dismiss", json={"ticket_id": 1, "type": "invalid"})
        assert res.status_code == 400

    def test_dismiss_nonexistent_ticket(self, client):
        res = client.put("/api/notifications/dismiss", json={"ticket_id": 99999, "type": "gate_review"})
        assert res.status_code == 404
...
class TestDismissedNotificationsFiltered:
    ...
    def test_dismissed_gate_review_not_shown(self, client):
        ...
        client.put("/api/notifications/dismiss", json={"ticket_id": t1, "type": "gate_review"})
    ...
    def test_dismissed_question_not_shown(self, client):
        ...
        client.put("/api/notifications/dismiss", json={"ticket_id": t1, "type": "question"})
...
class TestTimestampBasedDismissal:
    ...
    def test_dismissal_stays_hidden_for_existing_events(self, client):
        ...
        client.put("/api/notifications/dismiss", json={"ticket_id": ticket, "type": "gate_review"})
        notifs = _get_notifications(client)
        assert len(notifs) == 0
    ...
    def test_no_auto_clear_on_question_creation(self, client):
        ...
        res = client.post(
            f"/api/tickets/{ticket}/questions", json={"questions": [{"body": "New question after dismiss?"}]}
        )
