...
def _init_git_repo(path):
    ...
    subprocess.run(["git", "config", "user.email", "test@test.com"], cwd=path, capture_output=True, check=True)  # noqa: S603, S607
    subprocess.run(["git", "config", "user.name", "Test"], cwd=path, capture_output=True, check=True)  # noqa: S603, S607
    # Create initial commit
    readme = os.path.join(path, "README.md")
    with open(readme, "w") as f:
        f.write("# Test\n")
    subprocess.run(["git", "add", "README.md"], cwd=path, capture_output=True, check=True)  # noqa: S603, S607
    subprocess.run(["git", "commit", "-m", "initial"], cwd=path, capture_output=True, check=True)  # noqa: S603, S607

    # Setup origin remote pointing to self HEAD
    subprocess.run(["git", "branch", "-M", "main"], cwd=path, capture_output=True, check=True)  # noqa: S603, S607
    # Create a bare repo for origin
    bare_path = path + "-origin.git"
    subprocess.run(["git", "clone", "--bare", ".", bare_path], cwd=path, capture_output=True, check=True)  # noqa: S603, S607
    subprocess.run(["git", "remote", "add", "origin", bare_path], cwd=path, capture_output=True, check=False)  # noqa: S603, S607
    subprocess.run(["git", "remote", "set-url", "origin", bare_path], cwd=path, capture_output=True, check=True)  # noqa: S603, S607
    subprocess.run(["git", "push", "-u", "origin", "main"], cwd=path, capture_output=True, check=True)  # noqa: S603, S607
    # Set HEAD so symbolic-ref works
    subprocess.run(
        ["git", "symbolic-ref", "refs/remotes/origin/HEAD", "refs/remotes/origin/main"],  # noqa: S603, S607
        cwd=path,
        capture_output=True,
        check=True,
    )
...
def test_is_git_repo_true(tmp_path):
    ...
    os.makedirs(repo)
    subprocess.run(["git", "init"], cwd=repo, capture_output=True, check=True)  # noqa: S603, S607
...
def test_branch_exists_local(tmp_path):
    ...
    _init_git_repo(repo)
    # Create a branch
    subprocess.run(["git", "checkout", "-b", "test-branch"], cwd=repo, capture_output=True, check=True)  # noqa: S603, S607
    subprocess.run(["git", "checkout", "main"], cwd=repo, capture_output=True, check=True)  # noqa: S603, S607
...
def test_ensure_ticket_branch_creates_branch(tmp_path):
    ...
    current = subprocess.run(["git", "branch", "--show-current"], cwd=repo, capture_output=True, text=True)  # noqa: S603, S607
...
def test_ensure_ticket_branch_existing_branch(tmp_path):
    ...
    subprocess.run(["git", "checkout", "-b", branch_name], cwd=repo, capture_output=True, check=True)  # noqa: S603, S607
    subprocess.run(["git", "checkout", "main"], cwd=repo, capture_output=True, check=True)  # noqa: S603, S607
