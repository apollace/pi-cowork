...
class TestRestoreBackup:
    def test_restore_backup(self, client, backup_dir):
        ...
        res = client.post("/api/db-backup/restore", headers=_restore_headers(), data=_restore_body(backup_filename))
        assert res.status_code == 200
    ...
    def test_restore_nonexistent_file(self, client, backup_dir):
        ...
        res = client.post(
            "/api/db-backup/restore", headers=_restore_headers(), data=_restore_body("pi-cowork_20250101_120000.db")
        )
        assert res.status_code == 404

    def test_restore_invalid_filename(self, client, backup_dir):
        """Restore with path traversal → 400 error."""
        res = client.post("/api/db-backup/restore", headers=_restore_headers(), data=_restore_body("../etc/passwd"))
        assert res.status_code == 400

    def test_restore_missing_filename(self, client, backup_dir):
        """Restore without filename → 400 error."""
        res = client.post("/api/db-backup/restore", headers=_restore_headers(), data=json.dumps({"confirm": True}))
        assert res.status_code == 400
...
class TestDeleteBackup:
    ...
    def test_delete_nonexistent_backup(self, client, backup_dir):
        ...
        res = client.delete("/api/db-backup/delete", json={"filename": "pi-cowork_20250101_120000.db"})
        assert res.status_code == 404

    def test_delete_invalid_filename(self, client, backup_dir):
        """Delete with path traversal → 400 error."""
        res = client.delete("/api/db-backup/delete", json={"filename": "../../../etc/passwd"})
        assert res.status_code == 400
...
class TestSettingsMaxCount:
    ...
    def test_settings_max_count_updatable(self, client):
        ...
        res = client.put("/api/settings/db_backup_max_count", json={"value": "5"})
        assert res.status_code == 200
        assert res.get_json()["success"] is True
