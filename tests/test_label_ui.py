...
class TestLabelPopoverCSS:
    ...
    def test_label_popover_style_exists(self):
        ...
        assert '.label-popover' in css, (
            'Expected .label-popover styles in CSS for popover dropdown'
        )
    ...
    def test_label_popover_has_zindex(self):
        ...
        assert 'z-index' in body, (
            f'Expected z-index in .label-popover for overlay behavior, got:\n{body}'
        )
...
class TestLabelPillCSS:
    ...
    def test_label_pill_style_exists(self):
        ...
        assert '.label-pill' in css, (
            'Expected .label-pill styles in CSS for improved label readability'
        )
    ...
    def test_label_pill_has_min_width(self):
        ...
        assert 'min-width' in body, (
            f'Expected min-width in .label-pill for better click/read targets, got:\n{body}'
        )
...
class TestCardLabelPickerRemoved:
    ...
    def test_card_label_picker_removed_from_css(self):
        ...
        assert m is None, (
            'Expected .card-label-picker to be removed from CSS (replaced by .label-popover)'
        )
...
class TestBoardCardPopover:
    ...
    def test_app_js_no_card_label_picker_div(self):
        ...
        js = read(APP_JS_PATH)
        assert 'card-label-picker' not in js, (
            'Expected .card-label-picker reference removed from app.js'
        )

    def test_app_js_uses_popover_mode(self):
        """toggleCardLabels should use LabelPicker with popover: true."""
        js = read(APP_JS_PATH)
        assert 'popover: true' in js, (
            'Expected popover: true in LabelPicker instantiation in app.js'
        )
    ...
    def test_app_js_label_btn_has_id(self):
        ...
        js = read(APP_JS_PATH)
        assert 'card-label-btn-' in js, (
            'Expected card-label-btn- id on the label trigger button'
        )
    ...
    def test_app_js_pill_uses_label_pill_class(self):
        ...
        assert 'label-pill' in js, (
            'Expected label-pill class used in buildCard pill rendering'
        )
    ...
    def test_app_js_pill_improved_opacity(self):
        ...
        if m:
            ...
            assert '33' in m or '33' in js, (
                f'Expected label pill background opacity 33, found patterns: {m}'
            )
        # Check border opacity
        m2 = re.findall(r'border:1px solid \$\{escapeHtml\(l\.color\)\}(\w+)', js)
        if m2:
            assert '55' in m2 or '55' in js, (
                f'Expected label pill border opacity 55, found patterns: {m2}'
            )
...
class TestLabelPickerPopoverMode:
    ...
    def test_label_picker_has_popover_property(self):
        ...
        assert 'this.popover' in html, (
            'Expected LabelPicker to store popover option'
        )

    def test_label_picker_has_renderPopover(self):
        html = read(BASE_HTML_PATH)
        assert '_renderPopover' in html, (
            'Expected LabelPicker to have _renderPopover method'
        )

    def test_label_picker_has_closePopover(self):
        html = read(BASE_HTML_PATH)
        assert 'closePopover' in html, (
            'Expected LabelPicker to have closePopover method'
        )
    ...
    def test_label_picker_click_away_handler(self):
        ...
        html = read(BASE_HTML_PATH)
        assert '_closeHandler' in html, (
            'Expected click-away handler for popover close'
        )

    def test_label_picker_escape_handler(self):
        """Popover should have Escape key close handler."""
        html = read(BASE_HTML_PATH)
        assert "_escHandler" in html or "'Escape'" in html, (
            'Expected Escape key handler for popover close'
        )
...
class TestLabelPickerCollapsedCreate:
    ...
    def test_create_toggle_exists(self):
        html = read(BASE_HTML_PATH)
        assert 'label-picker-create-toggle' in html, (
            'Expected .label-picker-create-toggle for collapsed create section'
        )
...
class TestLabelPickerReRender:
    ...
    def test_reRender_method_exists(self):
        html = read(BASE_HTML_PATH)
        assert '_reRender()' in html or 'this._reRender' in html, (
            'Expected _reRender method in LabelPicker'
        )
    ...
    def test_reRender_dispatches_to_popover(self):
        ...
        assert 'this.popover' in html, (
            'Expected _reRender to check this.popover for dispatching'
        )
...
class TestTicketDetailPillImprovement:
    ...
    def test_ticket_detail_uses_label_pill(self):
        ...
        assert 'label-pill' in html, (
            'Expected label-pill class in ticket detail label rendering'
        )

    def test_ticket_detail_uses_improved_opacity(self):
        html = read(TICKET_DETAIL_PATH)
        # Check that the pill rendering uses opacity 33 instead of 22
        assert '}33' in html or 'l.color)}33' in html, (
            'Expected improved opacity (33) in ticket detail label pills'
        )
