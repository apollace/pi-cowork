...
def test_header_rerun_button_still_present(client, default_board):
    ...
    assert res.status_code == 200
    html = res.data.decode('utf-8')
    assert 'rerun-agent-btn' in html, \
        "The header re-run agent button should still be present"
...
def test_log_button_still_present_in_completed_runs(client, default_workflow, default_board):
    ...
    assert res.status_code == 200
    html = res.data.decode('utf-8')
    assert 'toggleRunLog' in html, \
        "The toggleRunLog function should still exist for the Log button"
