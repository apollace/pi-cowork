...
def test_workflow_git_enabled_default_false(client):
    res = client.post('/api/workflows', json={
        'name': 'GitTest WF',
        'description': 'Test git toggle',
    })
    assert res.status_code == 201
    wf_id = json.loads(res.data)['id']
    wf = json.loads(client.get(f'/api/workflows/{wf_id}').data)
    assert wf.get('git_enabled') in (0, False)


def test_workflow_git_enabled_create(client):
    res = client.post('/api/workflows', json={
        'name': 'GitEnabled WF',
        'description': 'Git on',
        'git_enabled': True,
...
def test_workflow_git_enabled_update(client):
    res = client.post('/api/workflows', json={
        'name': 'Toggle WF',
...
def test_workflow_git_enabled_in_list(client):
    res = client.post('/api/workflows', json={
        'name': 'ListGit WF',
        'git_enabled': True,
    })
    ...
    workflows = json.loads(client.get('/api/workflows').data)
    found = [w for w in workflows if w['id'] == wf_id]
    assert len(found) == 1
    assert found[0]['git_enabled'] in (1, True)


def test_board_includes_git_enabled(client, default_workflow):
    """Board API responses should include git_enabled from the linked workflow."""
    # Enable git on workflow
    client.put(f'/api/workflows/{default_workflow["id"]}', json={'git_enabled': True})

    # Verify boards list includes git_enabled
    boards = json.loads(client.get('/api/boards').data)
    default_board = next(b for b in boards if b['workflow_id'] == default_workflow['id'])
    assert default_board['git_enabled'] in (1, True)

    # Verify single board GET includes git_enabled
    res = client.get(f'/api/boards/{default_board["id"]}')
    board = json.loads(res.data)
    assert board['git_enabled'] in (1, True)
