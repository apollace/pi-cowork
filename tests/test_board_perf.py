...
class TestBoardListingBatchQueries:
    ...
    def test_labels_in_board_listing(self, client, default_board):
        ...
        l1 = json.loads(client.post('/api/labels', json={
            'name': 'bug', 'color': '#ff0000', 'workflow_id': wf_id,
        }).data)
        l2 = json.loads(client.post('/api/labels', json={
            'name': 'feature', 'color': '#00ff00', 'workflow_id': wf_id,
        }).data)
...
class TestMigrationIdempotency:
    ...
    def test_double_migration(self, client):
        ...
        with app.app_context():
            from pi_cowork.db import get_db, _migrate
            db = get_db()
