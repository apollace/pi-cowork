...
def test_import_workflow(client):
    workflow_json = {
        ...
        "agents": [
            {"name": "Tester", "description": "Test agent", "working_directory": "/tmp/test"}
        ],
        "statuses": [
        ...
        "transitions": [
            {"from_status_name": "Start", "to_status_name": "End", "instructions": "Finish the task"}
        ]
    }
...
def test_import_workflow_duplicate_name(client):
    workflow_json = {
        ...
        "agents": [
            {"name": "Agent1", "description": "d1", "working_directory": "/tmp/d1"}
        ],
        "statuses": [
...
def test_import_workflow_missing_default_status(client):
    workflow_json = {
        ...
        "agents": [],
        "statuses": [
            {"name": "S1", "sort_order": 1, "is_default": False, "is_terminal": False}
        ],
        "transitions": []
    }
    res = client.post('/api/workflows/import', json=workflow_json)
...
def test_import_workflow_invalid_status_ref(client):
    workflow_json = {
        ...
        "agents": [],
        "statuses": [
            {"name": "S1", "sort_order": 1, "is_default": True, "is_terminal": False}
        ],
        "transitions": [
            {"from_status_name": "S1", "to_status_name": "NonExistent", "instructions": "go"}
        ]
    }
    res = client.post('/api/workflows/import', json=workflow_json)
    assert res.status_code == 400
...
def test_import_export_roundtrip(client):
    ...
    workflow_json = {
        ...
        "agents": [
            {"name": "RTAgent", "description": "Roundtrip agent", "working_directory": "/tmp/rt"}
        ],
        "statuses": [
            {"name": "Open", "sort_order": 1, "is_default": True, "is_terminal": False, "agent_name": "RTAgent", "goal": "Get stuff done"},
            {"name": "Done", "sort_order": 2, "is_default": False, "is_terminal": True, "agent_name": None, "goal": None}
        ],
        "transitions": [
            {"from_status_name": "Open", "to_status_name": "Done", "instructions": "When complete"}
        ]
    }
    res = client.post('/api/workflows/import', json=workflow_json)
    assert res.status_code == 200
...
def test_import_with_status_model_and_thinking(client):
    workflow_json = {
        ...
        "statuses": [
            {"name": "S1", "sort_order": 1, "is_default": True, "is_terminal": False, "agent_name": "A1", "model": "gpt-4o", "thinking": "high"},
            {"name": "S2", "sort_order": 2, "is_default": False, "is_terminal": False, "agent_name": None, "model": None, "thinking": None}
        ],
        "transitions": []
    }
    res = client.post('/api/workflows/import', json=workflow_json)
...
def test_import_export_roundtrip_with_status_model_thinking(client):
    ...
    workflow_json = {
        ...
        "statuses": [
            {"name": "S1", "sort_order": 1, "is_default": True, "is_terminal": False, "agent_name": "A1", "model": "claude-3", "thinking": "xhigh"},
