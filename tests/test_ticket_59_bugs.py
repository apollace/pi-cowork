...
class TestDrainQueueFullContext:
    ...
    def test_drain_queue_agent_prompt_includes_board_name(self, client, default_workflow, default_board):
        ...
        with patch('app.subprocess.Popen', side_effect=capture_popen):
            with patch('pi_cowork.agents._is_our_process', return_value=False):
                with client.application.app_context():
                    from pi_cowork.agents import drain_queue, cleanup_runs
                    cleanup_runs()
                    drain_queue()
        ...
        assert f"board_id={default_board['id']}" in context_msg, \
            "Bug A: context message should include board_id"
...
class TestDrainQueueOldStatusId:
    ...
    def test_drain_queue_carries_old_status_id(self, client, default_workflow, default_board):
        ...
        with client.application.app_context():
            from pi_cowork.db import query_db as qdb, row_to_dict as rtd
            ...
            backlog_statuses = qdb("SELECT id FROM statuses WHERE is_default = 1 AND workflow_id = ?", (default_workflow['id'],))
    ...
    def test_drain_queue_status_transition_in_prompt(self, client, default_workflow, default_board):
        ...
        from_res = client.post('/api/statuses', json={
            'name': 'SrcTransition',
            'sort_order': 1,
            'agent_id': aid,
            'workflow_id': default_workflow['id'],
        ...
        to_res = client.post('/api/statuses', json={
            'name': 'DstTransition',
            'sort_order': 2,
            'agent_id': aid,
            'workflow_id': default_workflow['id'],
