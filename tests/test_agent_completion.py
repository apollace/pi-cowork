...
def test_watcher_thread_updates_db_on_success(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        ...
        db = get_db()
        run = db.execute(
            "SELECT status, exit_code FROM agent_runs WHERE ticket_id = ?",
            (tid,)
        ).fetchone()
...
def test_cleanup_marks_null_pid_as_failed(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        ...
        db = get_db()
        run = db.execute(
            "SELECT status FROM agent_runs WHERE ticket_id = ? AND pid IS NULL",
            (tid,)
        ).fetchone()
...
def test_cleanup_stale_run(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        ...
        db = get_db()
        db.execute(
            "UPDATE agent_runs SET started_at = datetime('now', '-3 hours') WHERE ticket_id = ?",
            (tid,)
        )
        db.commit()
    ...
    with client.application.app_context():
        ...
        db = get_db()
        run = db.execute(
            "SELECT status FROM agent_runs WHERE ticket_id = ?",
            (tid,)
        ).fetchone()
        assert dict(run)['status'] == 'completed'