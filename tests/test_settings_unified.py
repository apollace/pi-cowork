...
from pi_cowork.system_logs import cleanup_old_logs, add_log
...
def _create_workflow_and_board(client):
    ...
    res = client.post('/api/statuses', json={
        'name': 'Closed', 'sort_order': 2, 'is_default': False,
        'is_terminal': True, 'workflow_id': wf_id
    })
...
class TestPurgeTerminalLogs:
    ...
    def test_purge_deletes_log_dirs(self, client, tmp_path):
        ...
        status_res = client.post('/api/statuses', json={
            'name': 'Backlog', 'sort_order': 1, 'is_default': True,
            'is_terminal': False, 'workflow_id': wf_id
        })
        status_id = json.loads(status_res.data)['id']

        terminal_res = client.post('/api/statuses', json={
            'name': 'Closed', 'sort_order': 2, 'is_default': False,
            'is_terminal': True, 'workflow_id': wf_id
        })
        terminal_status_id = json.loads(terminal_res.data)['id']

        board_res = client.post('/api/boards', json={
            'name': 'Purge Board',
            'workflow_id': wf_id,
    ...
    def test_purge_does_not_delete_non_terminal_logs(self, client, tmp_path):
        ...
        status_res = client.post('/api/statuses', json={
            'name': 'Open', 'sort_order': 1, 'is_default': True,
            'is_terminal': False, 'workflow_id': wf_id
        })
        status_id = json.loads(status_res.data)['id']
