...
def test_approve_manual_gate_moves_ticket(client):
    ...
    with patch('app.subprocess.Popen'):
        res = client.put(f'/api/gate_reviews/{review_id}', json={
            'status': 'approved',
...
def test_delete_status_cascades_gates_and_reviews(client):
    ...
    gates = json.loads(client.get(f'/api/quality_gates?from_status_id={status_ids[0]}&to_status_id={gate_status_id}').data)
    assert len(gates) == 1
    reviews = json.loads(client.get(f'/api/gate_reviews?ticket_id={ticket_id}').data)
    assert len(reviews) >= 1
...
def test_reject_then_approve_manual_gate_allows_move(client):
    ...
    # 2) Reject the gate
    with patch('app.subprocess.Popen'):
        res = client.put(f'/api/gate_reviews/{review_id_1}', json={
            'status': 'rejected',
            'comment': 'Needs more work',
        }, headers=HUMAN_HEADERS)
    assert res.status_code == 200
    ...
    with patch('app.subprocess.Popen'):
        res = client.put(f'/api/gate_reviews/{review_id_2}', json={
            'status': 'approved',
        }, headers=HUMAN_HEADERS)
    assert res.status_code == 200
...
def test_priority_update_persisted_with_pending_manual_gate(client):
    ...
    with patch('app.subprocess.Popen'):
        res = client.put(f'/api/tickets/{ticket_id}', json={
