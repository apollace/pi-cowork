...
def test_spawn_success(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        app_module.run_db(
            "UPDATE agent_runs SET status = 'completed', completed_at = datetime('now') WHERE ticket_id = ?",
            (ticket["id"],),
        )
...
def test_spawn_queued_when_parallel_limit_reached(client, default_workflow, default_board):
    ...
    with patch("app.subprocess.Popen") as mock_popen, patch("pi_cowork.agents._is_our_process", return_value=True):
