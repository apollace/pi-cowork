...
class TestContainerSafeZone:
    ...
    def test_container_has_bottom_safe_zone_padding(self):
        ...
        body = m.group(1)
        assert "padding-bottom: 80px" in body, f"Expected 'padding-bottom: 80px' in .container, got:\n{body}"
    ...
    def test_container_mobile_retains_safe_zone_padding(self):
        ...
        assert "padding-bottom: 80px" in block, f"Expected 'padding-bottom: 80px' in mobile .container, got:\n{block}"
    ...
    def test_container_base_top_padding_unchanged(self):
        ...
        assert m, ".container rule not found in CSS"
        body = m.group(1)
        assert "padding: 1.5rem" in body, f"Expected 'padding: 1.5rem' in .container, got:\n{body}"
    ...
    def test_container_mobile_shorthand_padding_present(self):
        ...
        block = _find_media_block_containing(css, ".container {")
        assert block is not None, "Missing .container rule inside @media (max-width: 768px)"
        assert "padding: 1rem" in block, f"Expected 'padding: 1rem' in mobile .container, got:\n{block}"
...
class TestAssistantBubblePosition:
    ...
    def test_assistant_bubble_position_fixed(self):
        ...
        body = m.group(1)
        assert "position: fixed" in body, f"Expected 'position: fixed' in .assistant-bubble, got:\n{body}"
    ...
    def test_assistant_bubble_bottom_right_16px(self):
        ...
        assert m, ".assistant-bubble rule not found in CSS"
        body = m.group(1)
        assert "bottom: 1rem" in body, f"Expected 'bottom: 1rem' in .assistant-bubble, got:\n{body}"
        assert "right: 1rem" in body, f"Expected 'right: 1rem' in .assistant-bubble, got:\n{body}"
    ...
    def test_assistant_bubble_z_index_at_least_50(self):
        ...
        assert z >= 50, f"Expected z-index >= 50 in .assistant-bubble, got {z}"
    ...
    def test_assistant_bubble_dimensions_56px(self):
        ...
        assert m, ".assistant-bubble rule not found in CSS"
        body = m.group(1)
        assert "width: 3.5rem" in body, f"Expected 'width: 3.5rem' in .assistant-bubble, got:\n{body}"
        assert "height: 3.5rem" in body, f"Expected 'height: 3.5rem' in .assistant-bubble, got:\n{body}"
