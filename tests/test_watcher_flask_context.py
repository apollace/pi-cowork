...
from pi_cowork.agents import _watch_agent
...
def test_watcher_in_daemon_thread_without_request_context(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        ...
        cur = db.execute(
            ...
            (tid, aid, datetime.now(UTC).isoformat(), "running"),
        )
        db.commit()
    ...
    with client.application.app_context():
        ...
        db = get_db()
        run = db.execute("SELECT status, exit_code FROM agent_runs WHERE id = ?", (run_id,)).fetchone()
        row = dict(run)
...
def test_watcher_in_daemon_thread_marks_failed_on_nonzero_exit(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        ...
        db = get_db()
        events = db.execute("SELECT event_name FROM event_log WHERE event_name = ?", ("agent.failed",)).fetchall()
...
def test_watcher_in_daemon_thread_publishes_completed_event(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        ...
        db = get_db()
        events = db.execute(
            "SELECT event_name, payload FROM event_log WHERE event_name = ?", ("agent.completed",)
        ).fetchall()
...
def test_watcher_exception_still_marks_failed_in_daemon_thread(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        ...
        db = get_db()
        run = db.execute("SELECT status FROM agent_runs WHERE id = ?", (run_id,)).fetchone()
