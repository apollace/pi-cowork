...
class TestEventLogRetentionSetting:
    def test_setting_seeded_in_db(self, client):
        ...
        db = get_db()
        row = db.execute("SELECT value FROM settings WHERE key = 'event_log_retention_days'").fetchone()
...
class TestCleanupOldEventLogs:
    ...
    def test_preserves_recent_entries(self, client):
        ...
        cleanup_old_event_logs(max_age_days=30)
        # Should not delete recent entries
        rows = db.execute("SELECT * FROM event_log WHERE event_name = 'comment.added'").fetchall()
        assert len(rows) >= 1
...
class TestEventLogRetentionPrecedence:
    ...
    def test_db_setting_overrides_env(self, client, monkeypatch):
        ...
        rows = db.execute("SELECT * FROM event_log WHERE event_name = 'test.db_over_env'").fetchall()
        assert len(rows) == 1
    ...
    def test_env_used_when_no_db_setting(self, client, monkeypatch):
        ...
        rows = db.execute("SELECT * FROM event_log WHERE event_name = 'test.env_fallback_recent'").fetchall()
        assert len(rows) == 1
