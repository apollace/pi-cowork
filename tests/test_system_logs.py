...
class TestSystemLogsTable:
    def test_system_logs_table_exists(self, client):
        ...
        db = get_db()
        rows = db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='system_logs'").fetchall()
        assert len(rows) == 1
...
class TestGetSystemLogs:
    def _seed_logs(self, client, count=10):
        ...
        for i in range(count):
            ...
            add_log(level, action, f"Log entry {i}", details={"index": i}, ticket_id=ticket_id if i % 2 == 0 else None)
...
class TestLogRotation:
    def test_cleanup_removes_old_logs(self, client):
        ...
        remaining = db.execute("SELECT * FROM system_logs WHERE message = 'old log'").fetchall()
    ...
    def test_cleanup_keeps_recent_logs(self, client):
        ...
        db = get_db()
        remaining = db.execute("SELECT * FROM system_logs WHERE message = 'recent log'").fetchall()
...
class TestDBChangeLogging:
    ...
    def test_create_status_logged(self, client):
        ...
        wf_id = json.loads(res.data)["id"]
        res = client.post("/api/statuses", json={"name": "TestStatus", "sort_order": 1, "workflow_id": wf_id})
        assert res.status_code == 201
        from pi_cowork.db import get_db
...
class TestSystemLogsCSSVariables:
    def test_no_undefined_card_variable(self, client):
        ...
        template_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "templates", "system_logs.html")
        with open(template_path) as f:
            content = f.read()

        # var(--card) must not appear anywhere (it's undefined in :root)
        assert "var(--card)" not in content, (
            "var(--card) found in system_logs.html but --card is not defined in :root. Use var(--surface) instead."
        )
