...
def test_warm_spawn_same_status_says_still(client, default_workflow, default_board):
    ...
    with patch('app.subprocess.Popen', side_effect=capture_popen):
        with patch('app.os.path.isdir', return_value=True):
            ...
            from pi_cowork.agents import try_spawn_or_queue; from pi_cowork.db import get_db as _get_db
            with client.application.app_context():
                db = _get_db()
...
def test_stale_spawn_uses_full_context(client, default_workflow, default_board):
    ...
    with client.application.app_context():
        db = get_db()
        db.execute(
            "UPDATE tickets SET agent_last_spawned_at = datetime('now', '-2 hours') WHERE id = ?",
            (tid,)
        )
        db.commit()
...
def test_agent_prompt_includes_transitions(client, default_workflow, default_board):
    ...
    s3 = client.post('/api/statuses', json={
        'name': 'Stage3',
        'sort_order': 3,
        'workflow_id': default_workflow['id'],
    })
