...
def test_scheduler_creates_ticket_when_due(client, default_board):
    ...
    assert any(p for p in due_tickets[0].get("recurring_parents", []) if p["id"] == task_id)
...
def test_scheduler_updates_next_trigger_after_fire(client, default_board):
    ...
    old = None
    with client.application.app_context():
        old = query_db("SELECT next_trigger_at FROM recurring_tasks WHERE id = ?", (task_id,), one=True)
        models.process_recurring_tasks()

    with client.application.app_context():
        new_task = query_db("SELECT * FROM recurring_tasks WHERE id = ?", (task_id,), one=True)
...
def test_disabled_task_not_triggered(client, default_board):
    ...
    _run_db_in_ctx(client, "UPDATE recurring_tasks SET next_trigger_at = ?, enabled = 0 WHERE id = ?", (past, task_id))
...
def test_end_at_past_disables_task(client, default_board):
    ...
    _run_db_in_ctx(
        client, "UPDATE recurring_tasks SET next_trigger_at = ?, end_at = ? WHERE id = ?", (past, end_past, task_id)
    )
    ...
    with client.application.app_context():
        task = query_db("SELECT enabled, next_trigger_at FROM recurring_tasks WHERE id = ?", (task_id,), one=True)
        td = row_to_dict(task)
        assert td["enabled"] == 0
...
def test_start_at_in_future(client, default_board):
    ...
    res = _make_recurring(client, board_id, status["id"], cron="0 9 * * 1", start_at=future)
    assert res.status_code == 201
    data = json.loads(res.data)
    assert data["next_trigger_at"] is not None
