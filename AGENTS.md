...
- **Assistant reconnect across refresh** — When a user refreshes the page while an assistant response is streaming, the frontend (`assistant.js`) detects the open panel via `localStorage.assistantPanelOpen`, queries `/api/assistant/active-run`, and if a `running` run exists, reconnects via `/api/assistant/stream?run_id={id}`. The reconnect generator replays logged NDJSON events from the run log file (`{session_dir}/runs/run-{id}.log`), then tails the file while the run is active in `_ASSISTANT_RUNS`, and finally yields a synthetic terminal event from the DB once the run finalizes. This ensures the user sees the full streamed response even if the browser disconnected mid-stream.
...
- **Assistant panel initialization race** — In `assistant.js`, `openPanel()` fires several async operations (`loadConfig`, `loadHistory`, `loadSavedPrompts`, `checkActiveRun`). If these are not properly awaited, `loadHistory()` may overwrite `messagesEl.innerHTML` after `reconnectToRun()` has already created its streaming placeholder, causing the reconnect to "lose track" of the stream. Always await history loading before checking for an active run.
