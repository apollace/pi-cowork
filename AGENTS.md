...
- **Assistant NDJSON event normalization** — The pi CLI may emit wrapped events (`{"type":"message_update","assistantMessageEvent":{"type":"text_delta","delta":"..."}}`) or a `turn_end` event instead of the legacy flat format (`{"type":"text_delta","chunk":"..."}` / `{"type":"done"}`). `_normalize_ndjson_event()` in `pi_cowork/assistant.py` unwraps `message_update` events (mapping `delta` → `chunk`), converts `turn_end` → `done`, and leaves already-flat events untouched. When modifying the assistant streaming pipeline, always run `_normalize_ndjson_event()` before event-type dispatch so both wrapped and legacy events are handled.
