...
- **Assistant streaming via SSE** — `POST /api/assistant/chat` returns a `text/event-stream` response consumed by the frontend with `fetch` + `getReader()`. The backend spawns `pi --mode json` and parses NDJSON events from stdout. A reader thread pushes events to a `queue.Queue`; the Flask generator yields SSE frames. The generator's `finally` block writes the accumulated assistant text to the DB. To avoid `stream_with_context` GC issues in tests, the view captures `current_app._get_current_object()` and pushes a manual `_app.app_context()` inside `finally` for DB writes. In test mode (`app.config.get('TESTING')`), the generator is materialised into a byte string before `client.post` returns, so side effects (DB writes) are committed synchronously and `_ASSISTANT_RUNS` is cleaned up before the next request starts.
...
**Per-gate observation suppression** — Each quality gate has a `notify_on_failure` flag (default `true` for manual gates, `false` for CLI gates). The self-improvement subscribers `_on_gate_failed` and `_on_gate_review_rejected` skip creating an observation when `notify_on_failure=False`. Manual gate rejections publish only `GATE_REVIEW_REJECTED` (the redundant `GATE_FAILED` publish was removed). `TICKET_RERUN_DETECTED` is suppressed by `_on_ticket_rerun_detected` when a recent gate-failure/rejection comment exists on the ticket (within the last 10 minutes), preventing duplicate observations from system-initiated re-spawns after gate failures. User-initiated backward moves still create `[Rerun]` observations because they lack recent gate comments.
