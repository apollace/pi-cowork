...
- **Agent-restricted endpoints** — `AGENT_RESTRICTED_KEYS` in `pi_cowork/api_docs.py` defines endpoints that must never be exposed to agents even if explicitly configured: `gate_reviews_list`, `quality_gates_list`, `notifications_list`, `db_backup_restore`, `feedback_put`, `feedback_post`. These endpoints either allow agents to approve their own quality gates (defeating human oversight), modify feedback records that should be human-only, or perform destructive human-only actions.
