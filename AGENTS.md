...
- **`agent_feedback`** — structured feedback for agent self-improvement, linked to tickets and optionally to specific agent runs or gate reviews (columns: `id`, `ticket_id` FK → tickets ON DELETE CASCADE, `run_id` FK → agent_runs ON DELETE SET NULL, `gate_review_id` FK → gate_reviews ON DELETE SET NULL, `feedback_type` CHECK IN (`'gate_rejected'`, `'cli_failed'`, `'agent_killed'`, `'agent_rerun'`, `'run_feedback'`), `reason`, `expected_behavior`, `context_json`, `created_at`, `consumed_at`, `consumed_by_run_id`, `source_event`, `created_by`). Model helpers: `add_agent_feedback()`, `get_feedback_for_ticket()`, `get_unconsumed_feedback()`, `get_unconsumed_feedback_enriched()` (accepts `consumed`, `feedback_type`, `ticket_id`, `agent_id`, `board_id`, `workflow_id`, `date_from`, `date_to`, `search`, `limit`, `offset`), `get_feedback_count()` (same filters, returns total row count), `mark_feedback_consumed()`.

## Skills Architecture (pure filesystem)

Skills are **pure filesystem packages** — there is no DB table for skills. The `skills` and `agent_skills` tables were removed in Ticket #146. Built-in/system skills were added in Ticket #147. **Built-in skills auto-enable by default** for all agents and the assistant as of Ticket #151; **global skills also auto-enable for the assistant** as of Ticket #188.
...
- **Default enablement**: All skills (built-in, global, and workflow-scoped) are automatically attached to every agent unless explicitly excluded via `excluded_skill_names`. The assistant auto-enables **built-in and global skills** (not workflow-scoped). The `skill_names` column is kept for backward compatibility but no longer affects inclusion.
- **Effective skill resolution**: `get_effective_skill_names(agent_id, workflow_id)` computes `all_available_skills(workflow_id) - excluded_names`, where `all_available_skills` comes from `list_skills(workflow_id)`. For the assistant, `get_assistant_effective_skill_names()` computes `(built_in_names ∪ global_names) - excluded_names`.
...
9. **Agent API tokens (auth-enabled only)**: When `is_auth_enabled()` is true, each cold spawn creates or reuses a dedicated API token named `agent-<agent_id>-ticket-<ticket_id>`. The plaintext token is cached in the session directory (`agent_api_token`) so warm spawns reuse the same token. The token is tied to the first user in the database and is injected into the cold-spawn context message as `Headers: Authorization: Bearer <token>` right before the API docs block. Warm spawns omit the docs block (pi already has it from the previous session). All dedicated agent tokens are revoked from `api_tokens` when `auth_enabled` is toggled off via the settings API.
...
- API endpoints are trimmed to only what the agent needs (PUT ticket, POST comment, etc.). When auth is enabled, the `API:` block is preceded by `Headers: Authorization: Bearer <token>` using the agent's dedicated token. **Omitted on warm spawns** since `--continue` gives pi access from the previous session
...
| `/api/skills/import-github` | POST | Import skill from a public GitHub repository (JSON body: `url`, optional `workflow_id`). URL may point to repo root or a `tree/branch/subpath`. |

### Agents (scoped by workflow)
| Route | Method | Description |
|-------|--------|-------------|
| `/api/agents?workflow_id=<id>` | GET | List agents for a workflow (includes `skill_names` and `excluded_skill_names` arrays per agent) |
| `/api/agents` | POST | Create agent (requires `workflow_id`, optional `skill_names` and `excluded_skill_names` arrays) |
| `/api/agents/<id>` | GET/PUT/DELETE | Get / update / remove agent (`PUT` accepts optional `skill_names` and `excluded_skill_names` arrays; delete blocked if assigned to a status) |
...
| Route | Method | Description |
|-------|--------|-------------|
| `/api/feedback` | GET | List feedback rows (query: `consumed` tristate — missing = all, `true` = consumed only, `false` = unconsumed only; `feedback_type`, `ticket_id`, `agent_id`, `board_id`, `workflow_id`, `date_from`, `date_to`, `search`, `page`, `per_page`, `limit`). Returns paginated envelope `{feedback, total, page, per_page, total_pages}`. Each feedback item includes `board_id`, `board_name`, `workflow_id`, `workflow_name`. |
| `/api/feedback/<id>/preview` | GET | Return canonical structured JSON payload for a self-improvement agent (includes ticket, agent, run, gate review, board, workflow, reason, expected_behavior, context, created_at). `board` and `workflow` are nested objects with `id` and `name` when available, otherwise `null`. |
...
| Route | Method | Description |
|-------|--------|-------------|
| `/api/observations` | GET | List aggregated observations across event_log, system_logs, agent_runs, and gate_reviews (query: ticket_id, type, date_from, date_to, search, page, per_page) |
...
| Route | Method | Description |
|-------|--------|-------------|
| `/api/assistant/chat` | POST | Start a chat stream (SSE). Optional `board_id` scopes to a board. |
| `/api/assistant/stop` | POST | Signal the active assistant run to stop. |
| `/api/assistant/active-run` | GET | Get the latest running assistant run for a scope (or `null`). |
| `/api/assistant/stream?run_id=<id>` | GET | Reconnect SSE stream to an active or finalized run. |
| `/api/assistant/history` | GET | List persisted assistant messages. Optional `board_id` for board-scoped history. |
| `/api/assistant/compact` | POST | Compact conversation via `pi --mode rpc`, then clear messages and runs. |
| `/api/assistant/reset` | POST | Reset session via `pi --mode rpc`, then clear messages and runs. |
| `/api/assistant/config` | GET/PUT | Get / update assistant settings (enabled, model, thinking, working_directory, system_prompt, auto_context, api_endpoints, excluded_skill_names). |
| `/api/assistant/saved-prompts` | GET/POST | List / create saved prompt snippets. |
| `/api/assistant/saved-prompts/<id>` | PUT/DELETE | Update / delete saved prompt. |
...
- **Authentication is opt-in** — controlled by the `auth_enabled` setting (`PI_AUTH_ENABLED`, default `0`). When disabled, all routes are open. When enabled, HTML routes require a browser session (`session["user_id"]`); API routes require either a valid session or an `Authorization: Bearer <api_token>` header. Static assets, `/login`, `/api/auth/setup-needed`, `/api/auth/login`, `/api/auth/logout`, and `/api/events/stream` are exempt. `/api/auth/setup` is exempt only on first-run (no users exist); additionally, `PUT /api/settings/auth_enabled` is reachable without authentication when no users exist so the settings endpoint can return `400 "Create account first"` instead of a generic `401`. The SSE stream (`/api/events/stream`) accepts authentication via the browser session cookie or via `?token=<api_token>` in the query string because EventSource cannot set custom headers. `/api/assistant/*` endpoints are human-facing and require a browser session; API tokens are not accepted for them. `pi_cowork/auth.py` implements `is_auth_enabled()`, password hashing, session helpers, API token generation/validation, and the `require_auth()` `before_request` gate. `pi_cowork/agent_auth.py` manages dedicated agent API tokens (`agent-<agent_id>-ticket-<ticket_id>`) that are auto-created on cold spawns, cached per session, and revoked when auth is disabled. `current_user` is injected into every Jinja template. CLI quality gates receive the current request's API token as the `PI_AUTH_TOKEN` environment variable when auth is enabled and the request authenticated via a Bearer token, so gate commands can call back into the pi-cowork API.
...
- **Agent-restricted endpoints** — `AGENT_RESTRICTED_KEYS` in `pi_cowork/api_docs.py` defines endpoints that must never be exposed to agents even if explicitly configured: `gate_reviews_list`, `gate_review_put`, `quality_gates_list`, `notifications_list`, `notifications_dismiss`, `notifications_dismiss_all`, `feedback_put`, `feedback_post`, `agent_run_kill`, `settings_put`, `settings_purge_terminal_logs`, `db_backup_restore`. These endpoints either allow agents to approve their own quality gates (defeating human oversight), kill running agents, manage human notifications, modify human-only feedback records, change runtime config/limits, or perform destructive human-only actions. The assistant still receives restricted keys (`build_assistant_api_docs` intentionally does not filter them) since it is human-facing.
- **Registry completeness** — `ENDPOINT_REGISTRY` must contain an entry for every real `/api` REST route. `tests/test_api_docs.py::TestEndpointRegistryCompleteness` enumerates all Flask `app.url_map` rules under `/api/` (normalizing `<int:x>` → `{x}`) and asserts each has a matching registry entry, with no orphaned entries. Routes intentionally excluded from the registry are: `GET /api/endpoint-registry` (meta endpoint that lists the registry itself), `GET /api/agent_runs/{run_id}/stream` and `GET /api/events/stream` (SSE streams, not REST calls), and all `/api/assistant/*` routes (assistant self-referential control endpoints). When you add a new `/api` route, also add its registry entry (and add it to `AGENT_RESTRICTED_KEYS` if it is human-only) or the completeness test will fail.
...
- **Assistant reconnect across refresh** — On page load, `assistant.js` runs `initAssistant()`: it first queries `/api/assistant/active-run`; if a `running` run exists, the panel auto-opens and reconnects via `/api/assistant/stream?run_id={id}` even when `assistantPanelOpen` is not set (covers browser refresh and switching devices). Otherwise the panel opens only if `assistantPanelOpen` is `'1'`. `sendMessage()` and both stop-button handlers pass the active `board_id` (read from `localStorage.activeBoard`) in the request body so that the run scope matches `/api/assistant/active-run`. On the backend, `/api/assistant/stream` replays the run log and tails the active in-memory process; if the in-memory `_AssistantRun` is missing but the DB row is still `running` and the stored PID is still a `pi` process, the endpoint reattaches and continues streaming. `_read_log_frames` accumulates text/thinking deltas only for reattached runs (`proc is None`) so `_finalize_and_save` can write the correct `full_text` from the log. **`_finalize_and_save` always reconstructs `full_text` from the log file** (via `_reconstruct_text_from_log`), which contains the complete output regardless of whether the SSE generator was still consuming events when the browser disconnected. The reconnect generator yields a synthetic terminal event from the DB once the run finalizes, ensuring the user sees the full streamed response even after the browser disconnects mid-stream.
- **Assistant panel state persistence** — The assistant panel open/close state is persisted in `localStorage` as `assistantPanelOpen = '1'`. On page load, if the key is present, the panel auto-opens, loads history, and triggers `checkActiveRun()` → `reconnectToRun()`. Closing the panel removes the key.
...
- **Assistant streaming via SSE** — `POST /api/assistant/chat` returns a `text/event-stream` response consumed by the frontend with `fetch` + `getReader()`. The backend spawns `pi --mode json` and parses NDJSON events from stdout. A `_reader_thread` pushes parsed events to a `queue.Queue` and simultaneously writes every raw NDJSON line to a run log file (`{session_dir}/runs/run-{id}.log`). After stdout closes, `_reader_thread` also captures `proc.stderr` and writes it to the log as a `{"type":"_stderr","text":"..."}` line for diagnostics. The Flask generator (`_assistant_stream_generator`) yields SSE frames. The generator's `finally` block sets `generator_done`, which signals the `_watcher_thread` to call `_finalize_and_save`. The finalizer updates the `assistant_runs` DB row with `status`, `completed_at`, and `full_text`, and persists the assistant message to `assistant_messages`. **`full_text` is reconstructed from the log file** via `_reconstruct_text_from_log()` (which parses all `text_delta` chunks from the NDJSON log), NOT from `run.accumulated_text` — the latter is only populated while the SSE generator is actively consuming events and may be incomplete after a browser disconnect. `accumulated_text` is used as a fallback only when the log file is missing/unreadable. The finalizer also logs warnings when the process exits cleanly but produces empty or suspiciously short text (possible early-exit / model issue). Error events read stderr from the log file (not `proc.stderr`, which may have been consumed by `_reader_thread`). To avoid `stream_with_context` GC issues in tests, the view captures `current_app._get_current_object()` and pushes a manual `_app.app_context()` inside `finally` for DB writes. In test mode (`app.config.get('TESTING')`), the generator is materialised into a byte string before `client.post` returns, so side effects (DB writes) are committed synchronously and `_ASSISTANT_RUNS` is cleaned up before the next request starts.
- **Assistant conversation continuity via `--continue`** — The assistant uses pi's native session management (`--continue` flag) for multi-turn conversation continuity, NOT full-history replay. Each `POST /api/assistant/chat` call passes only the new user message (plus extra context like page_url, board context, skills metadata) as the CLI argument — previous conversation history is never re-sent. Pi resumes the latest session in the `--session-dir` automatically; `--continue` starts a fresh session when none exists. The `assistant_messages` DB table is display-only (powers the UI history view) and is NOT the source of truth for conversation context. Compact and reset RPC commands also use `--continue` to target the actual conversation session. This avoids argument length limits (~128KB), token waste from re-sending full history, and loss of message structure that occurred with the previous `USER:.../ASSISTANT:...` flattening approach.
- **Assistant NDJSON event normalization** — The pi CLI may emit wrapped events (`{"type":"message_update","assistantMessageEvent":{"type":"text_delta","delta":"..."}}`) or an `agent_end` event instead of the legacy flat format (`{"type":"text_delta","chunk":"..."}` / `{"type":"done"}`). `_normalize_ndjson_event()` in `pi_cowork/assistant.py` unwraps `message_update` events (mapping `delta` → `chunk`), converts `agent_end` → `done`, and leaves already-flat events untouched. `turn_end` is **not** mapped to `done` because the pi CLI can emit multiple `turn_end` events during multi-turn tool calls; only `agent_end` signals the true end of the stream. When modifying the assistant streaming pipeline, always run `_normalize_ndjson_event()` before event-type dispatch so both wrapped and legacy events are handled.
- **Assistant reconnect stream** — `GET /api/assistant/stream?run_id={id}` supports reconnecting to an active or recently finalized run. If the run is still in `_ASSISTANT_RUNS`, `_assistant_stream_generator_reconnect` replays existing log lines, tails the active process by reading new lines as they are appended (using a line-counting strategy rather than byte seeking), and yields a synthetic terminal event from the DB once finalized. If the run has already been finalized, it yields a single synthetic terminal event based on the DB row (`completed` → `done`, `stopped` → `stopped`, otherwise `error`). The frontend uses `full_text` (for `done`) or `partial` (for `stopped`) from the synthetic event **only when no `text_delta` events were streamed** (i.e. `placeholder.rawText` is empty). When `rawText` already has content from live streaming, it is authoritative and the server's `full_text` is not used to overwrite it — this prevents truncated server text from clobbering correctly-streamed text after a disconnect. The reconnect placeholder shows a "Reconnecting…" label (replaced by "Generating…" or "Thinking…" once the first delta arrives). This endpoint uses the same localhost/TESTING materialization pattern as `POST /api/assistant/chat`.
- **Assistant panel initialization race** — In `assistant.js`, `openPanel()` now accepts an optional active run id and awaits `loadConfig` → `loadHistory` → `loadSavedPrompts` before reconnecting, so `loadHistory()` cannot overwrite the streaming placeholder created by `reconnectToRun()`. `initAssistant()` checks for an active run first and only falls back to `localStorage.assistantPanelOpen`, preventing the reconnect placeholder from being clobbered by unawaited async setup.
...
- **Modal layout on narrow viewports** — `.modal-content` is a flex item when its parent `.modal` is opened with `display: flex`. Flex items have a default `min-width: auto`, which prevents them from shrinking below the intrinsic width of their children. A bare `<textarea>` inside `.modal-content` without explicit `width` uses its intrinsic width (~300px from `cols=20`), forcing the modal to overflow the viewport. Two CSS rules must be present: `min-width: 0` on `.modal-content`, and `.modal-content textarea { width: 100%; }` for any bare textareas not wrapped in `.form-card`.
...
- **New `/api/auth/*` routes are not blanket-exempt from auth middleware** — Only paths explicitly listed in `pi_cowork/auth.py:_EXEMPT_PATHS` bypass the middleware. New auth endpoints must either be added to `_EXEMPT_PATHS` (if they should be public, like setup/login) or be protected by `auth.require_session()` for human-only endpoints such as password/token management. They must also be added to `AGENT_RESTRICTED_KEYS` in `pi_cowork/api_docs.py` and registered in `ENDPOINT_REGISTRY`.
