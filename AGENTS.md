...
- **Assistant reconnect stream** — `GET /api/assistant/stream?run_id={id}` supports reconnecting to an active or recently finalized run. If the run is still in `_ASSISTANT_RUNS`, `_assistant_stream_generator_reconnect` replays existing log lines, tails the active process by reading new lines as they are appended (using a line-counting strategy rather than byte seeking), and yields a synthetic terminal event from the DB once finalized. If the run has already been finalized, it yields a single synthetic terminal event based on the DB row (`completed` → `done`, `stopped` → `stopped`, otherwise `error`). The frontend must use `full_text` (for `done`) or `partial` (for `stopped`) from the synthetic event because there may be no preceding `text_delta` events. This endpoint uses the same localhost/TESTING materialization pattern as `POST /api/assistant/chat`.
