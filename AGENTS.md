...
- **Authentication is opt-in** — controlled by the `auth_enabled` setting (`PI_AUTH_ENABLED`, default `0`). When disabled, all routes are open. When enabled, HTML routes require a browser session (`session["user_id"]`); API routes require either a valid session or an `Authorization: Bearer <api_token>` header. Static assets, `/login`, `/api/auth/*`, and `/api/events/stream` are exempt. `pi_cowork/auth.py` implements `is_auth_enabled()`, password hashing, session helpers, API token generation/validation, and the `require_auth()` `before_request` gate. `pi_cowork/agent_auth.py` manages dedicated agent API tokens (`agent-<agent_id>-ticket-<ticket_id>`) that are auto-created on cold spawns, cached per session, and revoked when auth is disabled. `current_user` is injected into every Jinja template.
